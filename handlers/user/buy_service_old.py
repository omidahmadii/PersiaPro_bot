...
async def choose_category(callback: CallbackQuery, state: FSMContext):
    ...
    # فهرست پلن‌های فعال براساس دسته
    plans = [
        p for p in get_all_plans()
        if normalize_category(p.get("category")) == category and _is_active(p)
    ]

    # برای standard / dual / custom_location / modem مستقیم می‌رویم سراغ مدت‌ها
...
async def choose_location(callback: CallbackQuery, state: FSMContext):
    ...
    # پلن‌های همان لوکیشن برای fixed_ip + فعال‌ها
    plans = [
        p for p in get_all_plans()
...
async def confirm_and_create(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        order_id = insert_order(
            ...
            price=plan["price"],
            status="active",
            volume_gb=plan.get("volume_gb"),  # ممکن است برای برخی پلن‌ها None باشد
        )
