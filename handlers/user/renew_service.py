...
def build_renew_access_blocked_text(current_balance: int, min_price: int) -> str:
    ...
    lines = [get_renew_disabled_text()]

    if min_price > 0:
        lines.extend([
            "",
            f"💳 موجودی فعلی شما: {format_price(current_balance)} تومان",
            f"💰 حداقل موجودی لازم برای باز شدن تمدید: {format_price(min_price)} تومان",
        ])
        if required_amount > 0:
            lines.append(f"➕ مبلغ موردنیاز برای باز شدن تمدید: {format_price(required_amount)} تومان")
...
async def renew_start(message: Message, state: FSMContext):
    ...
    prompt_text = (
        "فعلاً به خاطر کافی نبودن موجودی، فقط تمدیدهای در انتظار پرداختت را می‌توانی مدیریت کنی.\n"
        "سرویسی را انتخاب کن:"
