    ...
    for service in services:
        ...
        rows.append([
            InlineKeyboardButton(
                text=f"{service['username']} | {service.get('plan_name') or '-'} | {total_volume} گیگ",
...
async def extra_volume_entry(message: Message, state: FSMContext):
    ...
    await state.clear()
    await state.update_data(extra_volume_services=services, extra_volume_packages=packages)
...
async def extra_volume_choose_service(callback: CallbackQuery, state: FSMContext):
    ...
    packages = data.get("extra_volume_packages") or get_active_volume_packages()
