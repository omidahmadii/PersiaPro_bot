...
def build_dashboard_month_report(conn: sqlite3.Connection) -> str:
    ...
    return "\n".join(
        [
            ...
            f"تراکنش‌های تاییدشده این ماه: <b>{_fmt_num(approved_tx_row['cnt'])}</b>",
            f"جمع واریزی تاییدشده این ماه: <b>{_fmt_num(approved_tx_row['total'])}</b> تومان",
            ...
            f"تراکنش pending: <b>{_fmt_num(pending_transactions)}</b>",
            "",
...
def build_wallet_overview_report(conn: sqlite3.Connection) -> str:
    ...
    for row in raw_status_rows:
        ...
        normalized = "rejected" if status in {"reject", "rejected"} else status
        current = merged_status.setdefault(normalized, {"cnt": 0, "total": 0})
        current["cnt"] += row["cnt"]
    ...
    if merged_status:
        for status, values in sorted(merged_status.items(), key=lambda item: item[1]["cnt"], reverse=True):
            lines.append(f"• {status}: {values['cnt']} تراکنش | {_fmt_num(values['total'])} تومان")
    else:
        lines.append("• داده‌ای ثبت نشده.")
...
def build_user_detail_report(user_id: int) -> str | None:
    with _connect() as conn:
        ...
        approved_tx_summary = cur.fetchone()
    ...
    lines = [
        ...
        f"تراکنش‌های تاییدشده: <b>{_fmt_num(approved_tx_summary['cnt'])}</b>",
        f"جمع واریزی تاییدشده: <b>{_fmt_num(approved_tx_summary['total'])}</b> تومان",
        "",
        "وضعیت سفارش‌ها:",
    ]
    ...
    if recent_transactions:
        for row in recent_transactions:
            lines.append(
                f"• #{row['id']} | {escape(str(row['status']))} | {_fmt_num(row['amount'])} تومان | {escape(str(row['created_at'] or '-'))}"
            )
    else:
