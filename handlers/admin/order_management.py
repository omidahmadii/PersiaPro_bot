...
from services.db import get_order_with_plan, get_plans_for_admin, search_orders_for_admin, update_order_conversion_markers
...
def order_actions_keyboard(order: dict) -> InlineKeyboardMarkup:
    ...
    if str(order.get("status") or "").strip() not in FINAL_ORDER_STATUSES:
        conversion_marked = int(order.get("eligible_for_conversion") or 0) == 1
        rows.extend([
            ...
            [
                InlineKeyboardButton(
                    text="✅ طرح تبدیل: فعال" if conversion_marked else "♻️ طرح تبدیل: غیرفعال",
                    callback_data=f"order_mgmt|conversion_mark|{order_id}",
                )
            ],
...
@router.callback_query(F.data.startswith("order_mgmt|conversion_mark|"))
async def order_management_toggle_conversion_mark(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
    ...
    enabled = int(order.get("eligible_for_conversion") or 0) != 1
    updated = update_order_conversion_markers(order_id, enabled=enabled)
    if not updated:
        return await callback.answer("به‌روزرسانی انجام نشد.", show_alert=True)

    refreshed = get_order_with_plan(order_id)
    await state.set_state(None)
    await callback.message.answer(
        build_order_caption(refreshed),
        parse_mode="HTML",
        reply_markup=order_actions_keyboard(refreshed),
    )
    await callback.answer("نشان طرح تبدیل بروزرسانی شد.")
