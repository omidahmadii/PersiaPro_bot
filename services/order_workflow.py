...
FINAL_ORDER_STATUSES = {"canceled", "renewed", "archived"}
...
def _restore_speed_profile(username: Optional[str], group_name: Optional[str]) -> Optional[str]:
    if not username:
        return None
    try:
...
def _apply_live_plan_change(
    ...
    warning = _restore_speed_profile(username, group_name)
...
def _sync_volume_speed_state(
    ...
    return _restore_speed_profile(username, group_name), False
...
def purchase_volume_package(user_id: int, order_id: int, package_id: int) -> dict:
    ...
    if live_service:
        ibs_warning = _restore_speed_profile(username, group_name)
